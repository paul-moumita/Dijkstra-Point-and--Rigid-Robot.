...
def PrintPath(node,axis):
    # print("Print Path!")
    # if node.parent:
    print(node.parent)
    axis.scatter(node.x,node.y,s=10,c='red',alpha=1)
    return axis
...
def direct_to_go(weightx,weighty,cost,final,graph,g,q,queue,nodes,parent,seen,mins,box,rec,pol):
    ...
    for i in range(8):
                ...
                # temp = np.array(queue)
                # print("Ques cost: ",[x for _,_,x in queue])
                costw=cost[i]
                a=str(mapx)+' '+str(mapy)
                b=str(x)+' '+str(y)
                tup=(a,b,costw)
                ...
                if((tup not in graph) and (tupin not in graph) and (x>=0 and x<=((300/resolution)+radius) and y>=0 and y<=((200/resolution)+radius)) and (((x+d)<=(300/resolution)) and ((y+d)<=(200/resolution)) and ((x-d)>=(0/resolution)) and ((y-d)>=(0/resolution)))):
                    ...
                    if(((pathAvailability(x, y, arr, box, rec, pol, minBx, minBy, maxBx, maxBy, minFx, minFy, maxFx, maxFy, maxPx, minPx, maxPy, minPy))==1) and (x>=(arr[0]) and y>=(arr[1])) and ( x<=(arr[2]) and y<=(arr[3]) )):
                        ...
                        # print("b: ",b)
                        # print("Seen :", mapObj.seen)
                        # print("Condition :", b not in seen)
                        if(b not in seen):
                            mapObj.seen.append(b)
...
def dijkstra(graph,f,t,paths_to_goal,mapx,mapy,weightx,weighty,costw,final,box,rec,pol):
    ...
    q,seen,mins,queue= [(0,Node(f,0,mapx,mapy))],mapObj.seen, {f: 0},[(0,f)]
    count=0
    nodes_dijkstra.append(Node(f,0,mapx,mapy))
    node=''
    while (q and node!=t):
        # print("Nodes :",nodes_dijkstra)
    # for numb in range(0,2):
        ...
        direct_to_go(weightx,weighty,costw,final,graph,g,q,queue,nodes_dijkstra,v1,mapObj.seen,mins,box,rec,pol)
        # print("Seen: ", seen)

    
        
    ans= [v for  v in (nodes_dijkstra) if v.node == t]
    # print(ans)
    print("goal reached")

    if(len(ans)>0):
        return nodes_dijkstra,ans[0]
    else:
